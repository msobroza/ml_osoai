...
exp = Experiment("Sound Classifier")
try:
    import cPickle
...
def evaluate(model, input, target, stats_dir, probs_dir, iteration, register):
    """Evaluate a model.
    ...
    if register:
        exp.metric("AP_test", mAP)
        exp.metric("auc_test", mAUC)
    else:
        exp.metric("AP_train", mAP)
        exp.metric("auc_train", mAUC)
    logging.info(
...
def train(args):
    ...
    exp.param("balance_type", balance_type)
    exp.param("learning_rate", learning_rate)
    exp.param("model_type", model_type)
    exp.param("batch_size", batch_size)
    exp.param("dropout_rate", args.dropout_rate)
    exp.param("hidden_units", args.hidden_units)
    # Move model to gpu
    if cuda:
        model.cuda()
